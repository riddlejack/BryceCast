...
const requirementsPath = 'analysis/predictive-race-intelligence/requirements.txt';
...
export const runPredictiveRaceIntelligence = ({ cwd = repoRoot, stdio = 'inherit' } = {}) => {
  ...
  const dependencyCheck = spawnSync(python, ['-c', 'import numpy, pandas'], { cwd, stdio: 'pipe' });
  ...
  if (dependencyCheck.status !== 0) {
    throw new Error(
      `Predictive analytics requires numpy and pandas. Set BRYCECAST_ANALYTICS_PYTHON to a prepared Python, or install ${requirementsPath} for ${python}.`
    );
  }
