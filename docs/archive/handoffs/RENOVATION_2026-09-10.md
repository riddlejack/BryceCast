...
Monterey 2026 qualifying Group 2 has sparse early observations: the old recorder remained in Group 1's cooldown when Group 2 started. Early laps have roughly 15-second observations and one 30.147-second interval. The recorder now exits cooldown immediately on a new live session ID. The historical observations cannot be retroactively upgraded into measured one-second samples. Official section reports still retain all 16 sections for each of Bryce's eight completed qualifying laps, including the 74.2164-second fastest lap.

The qualifying heat maps use official section-time reports independently of replay cadence. Barber 2025 has a 15-second RaceTools heartbeat hole during lap 4, while its official lap 4 and all 11 sections remain complete. A separate Timing71 recording covers that interval at one-to-two-second cadence. Milwaukee 2025 has a 635-second heartbeat hole before Bryce's timed run; Timing71 covers the latter 391 seconds, starting before the reported green flag, and both official timed laps retain all nine sections. These supplements are separately downloadable with explicit clock alignment. A malformed qualifying checkered clock is rejected when it precedes the green flag, and the delay from a reported green flag to the first retained heartbeat is shown separately from the cadence inside the recording.
...
- Race and qualifying heat maps share one section with a session toggle, whole-session and lap-window scopes, individual laps, and visible average/median controls. Qualifying defaults to its full-session average and reports the number of valid observed laps; the race's eight-lap comparison threshold does not suppress short qualifying runs. Average mode now also computes the displayed section time as an arithmetic mean. Qualifying's field reference remains each driver's best comparable section and is labelled separately from Bryce's average.
...
See [Release and data refresh](../../operations/RELEASE_AND_DATA_REFRESH.md). `postrace:roll` now includes archive acquisition, source identity gates, replay rebuilding, the coverage ledger, official standings/history and final UI package generation in dependency order. Missing newly published archives remain explicit rather than blocking otherwise valid official results.
