...
- Live Race Control fields are not historical official result fields. Live rank/gap/points/pit/tire/overtake fields come from `/api/snapshot`, `/api/bryce`, `/api/timing`, and upstream Race Control timing. Historical results come from the career dataset and generated analytics artifacts.
- Points projection is required for live race v1 as a guarded card. The live source exposes `runningDriverPoints`, `totalDriverPoints`, and `totalEntrantPoints`; the card must render `live_only` when fields are populated and `unavailable` when not populated or not reconciled. It is not blocked from the v1 product, but it must not invent points from local standings math until the live race contract proves the rule.
...
| ID | Priority | Stakeholder | User question | Data source | Grain | Required fields | Availability | Caveat behavior | Visualization | Fallback | Owner |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `live_source_readiness` | v1 | operator, serious fans | Is this actually the right live INDY NXT session? | `/api/sources`, `/api/snapshot` | source/session | `endpoints[].id`, `state`, `readinessState`, `freshnessLabel`, `checkedAgeSeconds`, `modifiedAgeSeconds`, `heartbeat.Series`, `heartbeat.EventID`, `heartbeat.EventSessionID`, Bryce row identity | live_only | Timing endpoint can point to top-series; require INDY NXT heartbeat plus car 9 and `DriverID=2143` or exact Bryce identity. | source health banner + drawer | If `wrong_session`, show schedule/watch context and latest archived sample only as stale. | live API |
...
| `live_points_projection` | v1 | Bryce, family/friends, serious fans | What does the current running order mean for points? | `/api/bryce`, `/api/snapshot`, `/api/timing`, upstream `timingscoring-ris.json` | driver/session | `runningDriverPoints`, `totalDriverPoints`, `totalEntrantPoints`, `rank`, `liveRank`, `status`, `EventSessionID`, `sourceState` | live_only | Required v1 card. Display only Race Control point fields that are populated live; reconcile against official standings after session. Do not fabricate from local formula. | points delta card + compact field context | If point fields are null/missing/stale, show "Live points not populated by Race Control" and retain historical championship context separately. | live API |
...
| `debrief_section_strengths` | v1 | Bryce, team/engineering | Where was Bryce fast or weak by section? | `section_results_deep_by_race.csv`, `section_results_deep_by_section.csv` | session/lap | `sectionComparisonRows`, `medianSectionPercentile`, `topQuartileShare`, `bottomQuartileShare`, `bestSectionFamilies`, `weakestSectionFamilies`, `sourceState`, `caveat` | partial | 144/145 comparable INDY NXT sessions have both reports; `session_indy_nxt_2024_6325` is source-broken. Suppress low-denominator rows. | ranked bars or segment heatmap | If session is the corrupt holdout, show unavailable with source-broken gap. | historical analytics |
...
- Race Debrief can build the header, qualifying conversion, lap-position story, section strengths with holdout behavior, and source drawer.
