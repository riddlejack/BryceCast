...
"Watch this race unfold" can appear on 2024–2026 races, not just the three sessions
...
| `../semantic-layer/output/crosswalk/crosswalk-validation.json` | the 12 Timing71 2026 races + `canonicalSessionId` + GO/NO-GO + canonical winner check |
...
Per session, one capture-shaped snapshot row per **1-second virtual second** from the
green flag to the checker:
...
Feeds are ~6.7 MB total for 40 races (well under budget) and are deterministically
regenerable from the semantic-layer packs.
...
- `scripts/lib/replay-lake-feeds.mjs` — a source adapter with the sqlite overlay's
  ...
  feeds instead of the runner archive, and enforces the same live-guard.
...
- **(b) 2026 Timing71 GO races** — 12/12 watchable (the Road America R2 CONDITIONAL is
  served as-raced with the honesty line).
- **(c) practice / qualifying** — intentionally **not** shipped this pass.
