...
The source data — the raw career corpus, the canonical dataset, the timing captures and replay feeds — is kept out of Git history and published as a **data release** asset (`npm run data:restore`; see `DATA_RELEASE.md`), so it can be corrected or withdrawn cleanly. Operators get the same files, plus the deployment tooling that stays private, from a **private companion repository** that operators overlay onto a checkout of this one (git dir `<checkout>/.private.git`, work tree the checkout). Every path it tracks is matched by this repository's `.gitignore`, so a production checkout with the overlay present still reports a clean `git status`. See `docs/operations/RELEASE_AND_DATA_REFRESH.md`.
...
A checkout with neither the overlay nor a restored data release builds and serves the demo, but has no inputs for `/api/weather/*`, replay playback, or the post-race data roll.
...
Do not start source pollers, touch a production SQLite file, deploy, change jobs, send permission requests, commit anything private to this repository, or change this repository's visibility unless the user explicitly authorizes that action. Source data ships only through `npm run data:pack` releases, never in Git; SQLite and the wider research archive are unpublished. Never copy any of them into Git merely to make a test pass. Credit every source in `DATA_SOURCES.md`, and treat a publisher's removal request as binding.
