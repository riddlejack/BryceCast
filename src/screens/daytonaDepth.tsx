...
const StintTimeline = ({ pack }: { pack: ImsaDaytonaStintPack }) => {
  ...
  /* Lap position → the 24-hour axis (the car ran the full 24h over its laps). */
  const height = 84;
  const top = 26;
  const barH = 16;
  const gutterL = 4;
  ...
  return (
    <div>
      ...
      <div ref={ref} style={{ width: '100%', position: 'relative' }}>
        {width > 0 ? (
          <svg width={width} height={height} role="img" aria-label="When Bryce was in the No. 85 car across the 24 hours of Daytona">
            {/* the full race as a quiet track */}
            <rect x={plotL} y={top} width={plotR - plotL} height={barH} rx={5} fill="var(--surface-1)" />
            {/* hour ticks */}
            {hourTicks.map((hour) => {
              const hx = plotL + (hour / 24) * (plotR - plotL);
              return (
                <g key={hour}>
                  <line x1={hx} x2={hx} y1={top - 5} y2={top + barH + 5} stroke="var(--grid-hairline)" strokeWidth={1} />
                  <text x={hx} y={top + barH + 18} textAnchor={hour === 0 ? 'start' : hour === 24 ? 'end' : 'middle'} fill="var(--ink-muted)" fontFamily={chartFont} fontSize={10}>
                    {hour === 0 ? 'start' : hour === 24 ? '24h' : `${hour}h`}
                  </text>
                </g>
              );
            })}
      ...
      <p className="imsa-caption">
        Each blue block is one of his stints, placed across the car’s twenty-four hours; the gaps are his co-drivers at the wheel.
        Stint boundaries come from pit in/out laps on the official time card. Hover a stint for its laps and typical green pace.
      </p>
    </div>
  );
};
...
const CoDriverRoster = ({ pack }: { pack: ImsaDaytonaStintPack }) => {
  ...
  return (
    <div>
      ...
      <p className="imsa-caption">
        Laps and stints from the official time card. Bryce, the youngest in the lineup, took the No. 85 for {drivers.find((d) => d.role === 'Bryce')?.stintCount ?? 10} stints
